...
safeIndex :: [a] -> Int -> a -> a
safeIndex inp i a = if null inp then a else if i < (length inp) then inp!!i else a
...
stepE (SEQ xs)   y  = if (length xs) < (y+1) then (V {-exprID x-}, VOID) else (exprID (xs!!y), xs!!y)
stepE x          _  = (V {-exprID x-}, VOID)
